...
static  int  dm_allocate_loopdev(struct __loopdev_t * restrict  lpdevctr)  
{
  ...
  lpdevctr->_loopfd = open(DEVICE_LOOP_CTRL,  O_RDWR) ;
   ...
   asprintf(&lpdevctr->_loopname , "/dev/loop%i", lpdevctr->_devnum) ; 
   return 0 ;  
}
...
static void  dm_auto_remove_allocated_loopdev(struct __loopdev_t * lpdevctr)  
{ 
 ...
 /* 
  if(-1 == ioctl(lpdevctr->_loopfd, LOOP_CTL_REMOVE , lpdevctr->_devnum)){
    perr_r(ioctl, "@%s :%s" , __func__ ,strerror(*__errno_location())) ; 
  }
   */
}
